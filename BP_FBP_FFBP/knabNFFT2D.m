function result = knabNFFT2D(ExtrapVal, RHO_IN, COSALPHA_IN, INPUT_DATA, RHO_OUT, COSALPHA_OUT, B1, B2)
...
% --- Selva'RHO_OUT_SHIFTED window
P = 18;                                 % --- 2*P+1 is the number of samples involved in the sampling series
[Gn, kgn] = KnabWindow(T1, B1, P, size(INPUT_DATA.', 1));
[Gm, kgm] = KnabWindow(T2, B2, P, size(INPUT_DATA.', 2));
[Y, X] = meshgrid(Gm, Gn);    
G = X .* Y;

%--- NUFFT based Selva's approach to bandlimited signal interpolation with
%approximate prolate spheroidal wavefunction window
result = reshape(NFFT_Based_Selvas_Approach_for_Signal_Interp_2D(INPUT_DATA.', T1, T2, a, b, G, kgn, kgm, RHO_OUT_SHIFTED, COSALPHA_OUT_SHIFTED), size(RHO_OUT_SHIFTED, 1), size(COSALPHA_OUT_SHIFTED, 2));
