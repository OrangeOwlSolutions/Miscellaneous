function result = NFFT3_1D_Opt(data, x, s, c, K, Max_Num_PFs, SBP_factor)
...
Space_Bandwidth_Product=(SBP_factor*(2*pi-pi/c)*K);
[PSWFs P V_even V_odd K_Leg] = S0n(Space_Bandwidth_Product, 2 * Max_Num_PFs, (pi * s / (c * S)) / (2 * pi - pi / c), 1e-30);
PSWFs = PSWFs.';
...
%             for p=0:2:K_Leg-1
%                 phi_cap = phi_cap + alfa_prime * (sqrt(p+0.5)/(2*pi)) * sum(V_even(p/2+1,1:length(c_opt)).*c_opt) * ((2 * (-1i)^p .* sqrt(pi./(2.*alfa_prime*abs(c*x(l)*S/pi-k))) .* besselj(p+0.5,alfa_prime*abs(c*x(l)*S/pi-k)))).';
%             end
%         elseif (abs(round(c*x1(l))+c*M/2-k)<=K)
%             for p=0:2:K_Leg-1
%                 phi_cap = phi_cap + alfa_prime * (sqrt(p+0.5)/(2*pi)) * sum(V_even(p/2+1,1:length(c_opt)).*c_opt) * ((2 * (-1i)^p .* sqrt(pi./(2.*alfa_prime*abs(c*x(l)*S/pi-k+c*M))) .* besselj(p+0.5,alfa_prime*abs(c*x(l)*S/pi-k+c*M)))).';
%             end
%         elseif (abs(round(c*x1(l))-c*c*M/2-k)<=K)
%             for p=0:2:K_Leg-1
%                 phi_cap = phi_cap + alfa_prime * (sqrt(p+0.5)/(2*pi)) * sum(V_even(p/2+1,1:length(c_opt)).*c_opt) * ((2 * (-1i)^p .* sqrt(pi./(2.*alfa_prime*abs(c*x(l)*S/pi-k-c*M))) .* besselj(p+0.5,alfa_prime*abs(c*x(l)*S/pi-k-c*M)))).';
%             end
%         end
...
%             phi_cap=phi_cap.*exp(-1i*2*pi*s*M/(c*S));
        elseif (abs(round(c*x1(l))-2*M-k)<=K)
            for p=0:2:K_Leg-1
                phi_cap(l,k+M+1) = phi_cap(l,k+M+1) + alfa_prime * (sqrt(p+0.5)/(2*pi)) * sum(V_even(p/2+1,1:length(c_opt)).*c_opt) * ((2 * (-1i)^p .* sqrt(pi./(2.*alfa_prime*abs(c*x(l)*S/pi-k-c*M))) .* besselj(p+0.5,alfa_prime*abs(c*x(l)*S/pi-k-c*M)))).';
            end
...
result = NFFT1_1D_Opt(u, s * N / (c * S), c, K, Max_Num_PFs, SBP_factor) ./ phi2;
