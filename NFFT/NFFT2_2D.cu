...
/**************************/
extern "C" void Calculate_cuFFT_plan(const int N1, const int N2) { cufftSafeCall(cufftPlan2d(&NFFT2_2D_GPUplan, cc*N1, cc*N2, CUFFT_Z2Z)); }

/*************************/
/* FFTW PLAN CALCULATION */
/*************************/
extern "C" void Destroy_cuFFT_plan() { cufftSafeCall(cufftDestroy(NFFT2_2D_GPUplan)); }
...
__global__ void Kernel_Bessel(double * __restrict__ Bessel_vector, const int N)
{
	...
	if (i<N) {
		...
		Bessel_vector[i] = 1/(bessi0(K*sqrt(alfa*alfa-xi*xi)));
	}
}
...
/***************************/
extern "C" void NFFT2_2D_GPU(double2 * __restrict__ result, const double2 * __restrict__ data, const double * __restrict__ x, const double * __restrict__ y, const int N1, const int N2, const int M)
{
