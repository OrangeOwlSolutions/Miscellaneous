...
__global__ void Kernel_Bessel(T * __restrict__ Bessel_vector, const int N)
{
	...
	if (i<N) {
		...
		Bessel_vector[i] = static_cast<T>(1)/(bessi0(static_cast<T>(K)*sqrt(static_cast<T>(alfa*alfa)-xi*xi)));
	}
}
...
__global__ void series_terms(float2 temp_data, float2* __restrict__ result, const float r_cc_points1, const float cc_diff1, const float r_cc_points2, const float cc_diff2, const int N1, const int N2)
{
	...
	if(P<0.) {tempd=rsqrt(-P); phi_cap = (static_cast<float>(1./pi_double))*((sin(alfa/tempd))*tempd);  }
	else if(P>0.f) {tempd=rsqrt(P); phi_cap = static_cast<float>(1./pi_double)*((sinh(alfa/tempd))*tempd); }
	else phi_cap = static_cast<float>(alfa/pi_double);

	P = K*K-(cc_diff2-(n-K))*(cc_diff2-(n-K));

	if(P<0.) {tempd=rsqrt(-P); phi_cap = phi_cap*static_cast<float>(1./pi_double)*((sin(alfa/tempd))*tempd);  }
	else if(P>0.f) {tempd=rsqrt(P); phi_cap = phi_cap*static_cast<float>(1./pi_double)*((sinh(alfa/tempd))*tempd); }
	else phi_cap = static_cast<float>(phi_cap*alfa/pi_double);
...
__global__ void series_terms(double2 temp_data, double2* __restrict__ result, const double r_cc_points1, const double cc_diff1, const double r_cc_points2, const double cc_diff2, const int N1, const int N2)
{
	...
	if(P<0.) {tempd=rsqrt(-P); phi_cap = (1./pi_double)*((sin(alfa/tempd))*tempd);  }
	else if(P>0.) {tempd=rsqrt(P); phi_cap = (1./pi_double)*((sinh(alfa/tempd))*tempd); }
	else phi_cap = alfa/pi_double;

	P = K*K-(cc_diff2-(n-K))*(cc_diff2-(n-K));

	if(P<0.) {tempd=rsqrt(-P); phi_cap = phi_cap*(1./pi_double)*((sin(alfa/tempd))*tempd);  }
	else if(P>0.) {tempd=rsqrt(P); phi_cap = phi_cap*(1./pi_double)*((sinh(alfa/tempd))*tempd); }
	else phi_cap = phi_cap*alfa/pi_double;
...
#   else
	...
	if(i<M) {
		...
		for(int m=0; m<constant9_GPU; m++) {
			...
			if(P<0.) {tempd=rsqrt(-P); phi_cap1 = constant11_GPU_f*((sin(alfa/tempd))*tempd);  }
			else if(P>0.f) {tempd=rsqrt(P); phi_cap1 = constant11_GPU_f*((sinh(alfa/tempd))*tempd); }
			...
			for(int n=0; n<constant9_GPU; n++) {
				...
				if(P<0.f) {tempd=rsqrt(-P); phi_cap2 = phi_cap1*constant11_GPU_f*((sin(alfa/tempd))*tempd);  }
				else if(P>0.f) {tempd=rsqrt(P); phi_cap2 = phi_cap1*constant11_GPU_f*((sinh(alfa/tempd))*tempd); }
...
#   else
	...
	if(i<M) {
		...
		for(int m=0; m<constant9_GPU; m++) {
			...
			if(P<0.) {tempd=rsqrt(-P); phi_cap1 = constant11_GPU*((sin(alfa/tempd))*tempd);  }
			else if(P>0.) {tempd=rsqrt(P); phi_cap1 = constant11_GPU*((sinh(alfa/tempd))*tempd); }
			...
			for(int n=0; n<constant9_GPU; n++) {
				...
				if(P<0.) {tempd=rsqrt(-P); phi_cap2 = phi_cap1*constant11_GPU*((sin(alfa/tempd))*tempd);  }
				else if(P>0.) {tempd=rsqrt(P); phi_cap2 = phi_cap1*constant11_GPU*((sinh(alfa/tempd))*tempd); }
