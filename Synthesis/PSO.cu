...
/* PSO PARAMETERS */
/******************/
#define numberOfParticles		250							// --- Number of individuals
#define numberOfUnknowns		56							// --- Number of unknowns
#define numberOfGenerations     150							// --- Number of iterations
...
extern const float	aap;									// Reflectarray semi-dimension along the x-axis
extern const float	bap;									// Reflectarray semi-dimension along the y-axis
...
//		MAXVALS[k] =  0.004f;
	...
	MINVALS[Num_unknowns_phases+Num_unknowns_x] = aap;
	MAXVALS[Num_unknowns_phases+Num_unknowns_x] = (6.f / 5.f) * aap;
	MINVALS[Num_unknowns_phases+Num_unknowns_x*Num_unknowns_y+1] = bap;
	MAXVALS[Num_unknowns_phases+Num_unknowns_x*Num_unknowns_y+1] = (6.f / 5.f) * bap;
...
void h_findGlobalBest(float * __restrict__ h_globalBestFitness, unsigned int * __restrict__ h_globalBestID, 
	...
	//saveGPUrealtxt(d_personal_best_functional, "C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\d_personal_best_functional.txt", numberOfParticles);
...
void CostFunctionalCalculation(float * __restrict__ indata, float * __restrict__ outdata) {
	...
	for (int k = 0; k < numberOfParticles; k++) {
		...
		if ((h_min_distances[k] < 0.49f * lambda_f) || (h_max_distances[k] > 0.70f * lambda_f)) h_functionals[k] = FLT_MAX;
		
#ifdef DEBUG_PRINTING
		printf("%f %f %f\n", h_min_distances[k], h_max_distances[k], h_functionals[k]);
#endif	
	}
...
void h_PSO_Optimize(void)
{
	...
	for(generationNumber = 1; generationNumber < numberOfGenerations; ++generationNumber){
		...
		printf("Generation number = %i; \t Partial minimum found = %e\n", generationNumber, 10.*log10(h_globalBestFitness));
...
//        printf("GPU memory usage: used = %f, free = %f MB, total = %f MB\n", used_db/1024.0/1024.0, free_db/1024.0/1024.0, total_db/1024.0/1024.0);
	...
	saveGPUcomplextxt(d_far_field,	"/home/angelo/cuda-workspace/ParticleSwarmSynthesis/Release/Far_Field_NUFFT.txt", (2 * Nu) * (2 * Nv));
	// --- Solution:
	saveGPUrealtxt(d_best_personal_positions + h_globalBestID * actualParticleSize,	"/home/angelo/cuda-workspace/ParticleSwarmSynthesis/Release/Unknowns.txt", numberOfUnknowns);
