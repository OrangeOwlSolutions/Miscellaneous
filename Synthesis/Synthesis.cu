// --- ATTENTION: float AND double VERSIONS ARE NOT ALIGNED. float VERSIONS ARE THE NEWEST!
...
float2_ * raFarFieldCalculationSaving(float *d_in) {
	...
	saveGPUrealtxt(d_phases,	"C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\PHASES_CUDA.txt", M_x * M_y);
	...
	saveGPUrealtxt(d_X,	"C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\X_CUDA.txt", M_x * M_y);
	saveGPUrealtxt(d_Y,	"C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\Y_CUDA.txt", M_x * M_y);
	saveGPUrealtxt(d_Z,	"C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\Z_CUDA.txt", M_x * M_y);

	// --- Calculating the aperture field
	float2_ *d_excitations;		gpuErrchk(cudaMalloc(&d_excitations, M_x * M_y * sizeof(float2_)));
	calculateApertureField(d_X, d_Y, d_Z, d_phases, d_excitations, alfa_f, beta_f, mfact_f, 1., M_x * M_y);

	saveGPUcomplextxt(d_excitations,	"C:\\Users\\angelo\\Documents\\CEM\\ParticleSwarm\\ParticleSwarmSynthesis\\ParticleSwarmSynthesisMatlab\\Excitations_CUDA.txt", M_x * M_y);
