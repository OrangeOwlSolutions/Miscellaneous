...
/*************************************************************************************************/
/* THRUST FUNCTOR FOR COMPUTING THE GEOMETRICAL INTERSECTION BETWEEN A RAY AND BVH ON THE DEVICE */
/*************************************************************************************************/
struct device_compute_geo_inters_with_bvh {
    ...
    thrust::device_vector<int>::const_iterator      rmap_begin;

	thrust::device_vector<int2>::const_iterator     N0_begin;
    thrust::device_vector<float4>::const_iterator   N1_begin;
    thrust::device_vector<float4>::const_iterator   N2_begin;
    thrust::device_vector<float4>::const_iterator   N3_begin;
    
    aabb											scn_aabb;
    
    //
    device_compute_geo_inters_with_bvh(thrust::device_vector<int4>::const_iterator faces_begin_, thrust::device_vector<float4>::const_iterator verts_begin_, 
		                               thrust::device_vector<int>::const_iterator rmap_begin_, thrust::device_vector<int2>::const_iterator N0_begin_,
									   thrust::device_vector<float4>::const_iterator N1_begin_, thrust::device_vector<float4>::const_iterator N2_begin_, 
									   thrust::device_vector<float4>::const_iterator N3_begin_, aabb scn_aabb_) :
    ...
    __device__ float near_fun(const float2 &near_far) { return near_far.x; }
    __device__ float far_fun (const float2 &near_far) { return near_far.y; } 
    ...
    __device__ geointer operator ()(const vec3 &orig, const vec3 &dir) {
		...
		//printf("scn_near_far.x %f scn_near_far.y %f\n", scn_near_far.x, scn_near_far.y);
        ...
        if(near_fun(scn_near_far) <= far_fun(scn_near_far)){ // --- If the ray hit the scene bb 
            ...
            while(stack_top > 0) {
                ...
                for(thrust::device_vector<int>::const_iterator rmap_it = rmap_begin + N0.x; rmap_it != rmap_begin + N0.y; ++rmap_it) {
...
void device_bvh_trace(device_ray_list::R0_R1_iterator begin, device_ray_list::R0_R1_iterator end, thrust::device_vector<int4>::const_iterator faces_begin, 
    ...
    //printf("Here device_bvh_trace\n");
						  
	thrust::transform(begin, end, geointer_begin, device_compute_geo_inters_with_bvh(faces_begin, verts_begin, rmap_begin, N0_begin, N1_begin, N2_begin,
					  N3_begin, scn_aabb));
